...
this.currentItem);if(this.options.changedCallback)this.options.changedCallback(this,this.currentItem)}};Plugin.prototype.nextItem=function(){if(this.inTransition){this.commandQueue.push("nextItem");return}if(this.options.infiniteScroll||this.currentItem<this.numSliderItems-1){this.currentItem+=1;this.currentItem=this._wrapIndex(this.currentItem);this.setItem(this.currentItem);if(this.options.userChangedCallback)this.options.userChangedCallback(this,this.currentItem)}};Plugin.prototype.prevItem=function(){if(this.inTransition){this.commandQueue.push("prevItem");
return}if(this.options.infiniteScroll||this.currentItem>=1){this.currentItem-=1;this.currentItem=this._wrapIndex(this.currentItem);this.setItem(this.currentItem);if(this.options.userChangedCallback)this.options.userChangedCallback(this,this.currentItem)}};Plugin.prototype.doCommandQueue=function(){if(this.commandQueue.length==0)return;var cmd=this.commandQueue.splice(0,1)[0];this[cmd]()};Plugin.prototype.refresh=function(){this.items=$(this.element).children("li");this.numSliderItems=this.items.length;
this.setItem(this.currentItem)};Plugin.prototype._wrapIndex=function(n){if(n<0)n+=this.numSliderItems;if(n>=this.numSliderItems)n-=this.numSliderItems;return n};$.fn[pluginName]=function(options){return this.each(function(){if(!$.data(this,"plugin_"+pluginName))$.data(this,"plugin_"+pluginName,new Plugin(this,options))})};function _supportsTransitions(){var b=document.body||document.documentElement;var s=b.style;var p="transition";if(typeof s[p]=="string")return true;v=["Moz","Webkit","Khtml","O",
"ms"],p=p.charAt(0).toUpperCase()+p.substr(1);for(var i=0;i<v.length;i++)if(typeof s[v[i]+p]=="string")return true;return false}})(jQuery,window,document);